...
### ⚠️ Streamlit WebUI 弃用通知

> **Streamlit WebUI (`web/` 目录) 已正式弃用**，将在未来版本中移除。
>
> **请迁移到新的 Vue.js 前端**，它提供更丰富的功能和更好的用户体验：
> - 🚀 批量分析、股票筛选、自选股、模拟交易
> - 🎨 深色主题、响应式设计
> - 🔔 实时通知（SSE/WebSocket）
> - 📊 完整的系统管理功能
>
> **迁移指南**：启动 Streamlit 后，侧边栏底部可查看详细的迁移指南。
>
> **新前端访问地址**：默认 `http://localhost:3000`（可通过 `VUE_FRONTEND_URL` 环境变量配置）
