...
class AnalysisParameters(BaseModel):
    ...
    language: str = "zh-CN"
    # 模型配置
    quick_analysis_model: str | None = "qwen-turbo"
    deep_analysis_model: str | None = "qwen-max"
...
class BatchAnalysisRequest(BaseModel):
    ...
    stock_codes: list[str] | None = Field(
        None, min_items=1, max_items=10, description="股票代码列表(已废弃,使用symbols，最多10个)"
    )
    parameters: AnalysisParameters | None = None
...
class AnalysisTaskResponse(BaseModel):
    ...
    # 时间估算字段
    elapsed_time: float | None = Field(default=None, description="已用时间（秒）")
    remaining_time: float | None = Field(default=None, description="预计剩余时间（秒）")
    estimated_total_time: float | None = Field(default=None, description="预估总时长（秒）")
