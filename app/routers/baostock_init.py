...
_initialization_status = ThreadSafeState(
    {"is_running": False, "current_task": None, "stats": None, "start_time": None, "last_update": None}
)
...
async def start_full_initialization(request: InitializationRequest, background_tasks: BackgroundTasks):
    """启动完整初始化"""
    if _initialization_status["is_running"]:
        raise HTTPException(status_code=409, detail="初始化任务正在运行中，请等待完成后再试")
    ...
    try:
        ...
        # 启动后台任务
        background_tasks.add_task(_run_full_initialization_task, request.historical_days, request.force, task_id)
...
async def start_basic_initialization(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return InitializationResponse(
            success=True, message="基础初始化任务已启动", task_id=task_id, data={"estimated_duration": "10-20分钟"}
        )
...
async def get_initialization_status():
    """获取初始化状态"""
    try:
        status = _initialization_status.snapshot()
        ...
        if status["start_time"]:
            if status["is_running"]:
                ...
                duration = (
                    (status["last_update"] - status["start_time"]).total_seconds() if status["last_update"] else 0
                )
...
async def stop_initialization():
    ...
    if not _initialization_status["is_running"]:
        return {"success": True, "message": "没有正在运行的初始化任务", "data": {"was_running": False}}

    try:
        # 更新状态
        _initialization_status.update({"is_running": False, "current_task": None, "last_update": datetime.now()})

        return {"success": True, "message": "初始化任务已停止", "data": {"was_running": True}}
...
async def _run_full_initialization_task(historical_days: int, force: bool, task_id: str):
    ...
    try:
        ...
        # 更新状态
        _initialization_status.update({"is_running": False, "stats": stats, "last_update": datetime.now()})
        ...
        _initialization_status.update({"is_running": False, "last_update": datetime.now()})
