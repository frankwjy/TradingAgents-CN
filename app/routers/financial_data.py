...
async def get_latest_financial_data(
    ...
    try:
        ...
        if result:
            ...
            return ok(success=False, data=None, message="未找到财务数据")
...
async def start_financial_sync(request: FinancialSyncRequest, background_tasks: BackgroundTasks) -> dict:
    ...
    try:
        ...
        return ok(data={"task_started": True, "config": request.dict()}, message="财务数据同步任务已启动")
...
async def sync_single_stock_financial(request: SingleStockSyncRequest) -> dict:
    ...
    try:
        ...
        results = await service.sync_single_stock(symbol=request.symbol, data_sources=request.data_sources)
