...
async def get_stock_info(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票信息失败: {str(e)}")
...
async def get_stock_quote(market: str, code: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票行情失败: {str(e)}")
...
async def get_stock_daily_quotes(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取历史K线失败: {str(e)}")
