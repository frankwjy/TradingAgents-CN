...
async def get_operation_logs(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取操作日志列表失败: {str(e)}")
...
async def get_operation_log_stats(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取操作日志统计失败: {str(e)}")
...
async def get_operation_log_detail(log_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取操作日志详情失败: {str(e)}")
...
async def clear_operation_logs(request: ClearLogsRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"清空操作日志失败: {str(e)}")
...
async def create_operation_log(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"创建操作日志失败: {str(e)}")
...
async def export_logs_csv(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"导出操作日志CSV失败: {str(e)}")
