...
async def get_stock_basic_info(symbol: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票基础信息失败: {str(e)}")
...
async def get_market_quotes(symbol: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取实时行情失败: {str(e)}")
...
async def get_stock_list(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票列表失败: {str(e)}")
...
async def get_combined_stock_data(symbol: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票综合数据失败: {str(e)}")
...
async def search_stocks(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"搜索股票失败: {str(e)}")
...
async def get_market_summary(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取市场概览失败: {str(e)}")
...
async def get_quotes_sync_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取同步状态失败: {str(e)}")
