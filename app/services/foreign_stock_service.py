...
class ForeignStockService:
    ...
    # 缓存时间配置（秒，从配置文件加载）
    CACHE_TTL = (
        get_mapping_loader().get_cache_ttl_config()
        if get_mapping_loader().get_cache_ttl_config()
        else {
    ...
    async def _get_hk_quote(self, code: str, force_refresh: bool = False) -> dict:
        ...
        if not force_refresh:
            cache_key = self.cache.find_cached_stock_data(symbol=code, data_source="hk_realtime_quote")
        ...
        async with lock:
            ...
            self.cache.save_stock_data(
                symbol=code, data=json.dumps(formatted_data, ensure_ascii=False), data_source="hk_realtime_quote"
            )
    ...
    async def _get_us_quote(self, code: str, force_refresh: bool = False) -> dict:
        ...
        # 1. 检查缓存（除非强制刷新）
        if not force_refresh:
            cache_key = self.cache.find_cached_stock_data(symbol=code, data_source="us_realtime_quote")
        ...
        async with lock:
            ...
            # 6. 保存到缓存
            self.cache.save_stock_data(
                symbol=code, data=json.dumps(formatted_data, ensure_ascii=False), data_source="us_realtime_quote"
            )
    ...
    async def _get_hk_info(self, code: str, force_refresh: bool = False) -> dict:
        ...
        # 1. 检查缓存（除非强制刷新）
        if not force_refresh:
            cache_key = self.cache.find_cached_stock_data(symbol=code, data_source="hk_basic_info")
        ...
        # 5. 保存到缓存
        self.cache.save_stock_data(
            symbol=code, data=json.dumps(formatted_data, ensure_ascii=False), data_source="hk_basic_info"
        )
    ...
    async def _get_us_info(self, code: str, force_refresh: bool = False) -> dict:
        ...
        # 1. 检查缓存（除非强制刷新）
        if not force_refresh:
            cache_key = self.cache.find_cached_stock_data(symbol=code, data_source="us_basic_info")
        ...
        # 5. 保存到缓存
        self.cache.save_stock_data(
            symbol=code, data=json.dumps(formatted_data, ensure_ascii=False), data_source="us_basic_info"
        )
    ...
    async def _get_hk_kline(self, code: str, period: str, limit: int, force_refresh: bool = False) -> list[dict]:
        ...
        if not force_refresh:
            cache_key = self.cache.find_cached_stock_data(symbol=code, data_source=cache_key_str)
        ...
        # 4. 保存到缓存
        self.cache.save_stock_data(
            symbol=code, data=json.dumps(kline_data, ensure_ascii=False), data_source=cache_key_str
        )
    ...
    def _get_us_kline_from_yfinance(self, code: str, period: str, limit: int) -> list[dict]:
        ...
        # 周期映射
        period_map = {"day": "1d", "week": "1wk", "month": "1mo", "5m": "5m", "15m": "15m", "30m": "30m", "60m": "60m"}
    ...
    def _get_us_kline_from_finnhub(self, code: str, period: str, limit: int) -> list[dict]:
        ...
        # 获取K线数据
        candles = client.stock_candles(code.upper(), resolution, int(start_date.timestamp()), int(end_date.timestamp()))
    ...
    async def get_hk_news(self, code: str, days: int = 2, limit: int = 50) -> dict:
        ...
        # 4. 构建返回数据
        result = {"code": code, "days": days, "limit": limit, "source": data_source, "items": news_data}

        # 5. 缓存数据
        self.cache.save_stock_data(symbol=code, data=json.dumps(result, ensure_ascii=False), data_source=cache_key_str)
    ...
    def _get_us_news_from_finnhub(self, code: str, days: int, limit: int) -> list[dict]:
        ...
        news = client.company_news(
            code.upper(), _from=start_date.strftime("%Y-%m-%d"), to=end_date.strftime("%Y-%m-%d")
        )
    ...
    def _get_hk_news_from_finnhub(self, code: str, days: int, limit: int) -> list[dict]:
        ...
        # 获取公司新闻
        news = client.company_news(hk_symbol, _from=start_date.strftime("%Y-%m-%d"), to=end_date.strftime("%Y-%m-%d"))
    ...
    def _get_hk_kline_from_finnhub(self, code: str, period: str, limit: int) -> list[dict]:
        ...
        # 周期映射
        resolution_map = {"day": "D", "week": "W", "month": "M", "5m": "5", "15m": "15", "30m": "30", "60m": "60"}
