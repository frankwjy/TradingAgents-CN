...
class SimpleAnalysisService:
    ...
    async def execute_analysis_background(self, task_id: str, user_id: str, request: SingleAnalysisRequest):
        ...
        try:
            ...
            await asyncio.to_thread(
                progress_tracker.update_progress, {"progress_percentage": 10, "last_message": "🚀 开始股票分析"}
            )
            ...
            # 数据准备阶段（在线程中执行）
            await asyncio.to_thread(
                progress_tracker.update_progress, {"progress_percentage": 20, "last_message": "🔧 检查环境配置"}
            )
            await self.memory_manager.update_task_status(
                task_id=task_id,
                status=TaskStatus.RUNNING,
        ...
        try:
            ...
            def simulate_progress():
                ...
                try:
                    ...
                    # 辩论阶段 - 根据5个级别确定辩论轮次
                    research_depth = request.parameters.research_depth if request.parameters else "标准"
                    if research_depth == "快速" or research_depth == "基础" or research_depth == "标准":
                        debate_rounds = 1
                    elif research_depth == "深度":
                        debate_rounds = 2
                    elif research_depth == "全面":
                    ...
                    if progress_pct is not None:
                        ...
                        if int(progress_pct) > current_progress:
                            ...
                            progress_tracker.update_progress(
                                {"progress_percentage": int(progress_pct), "last_message": message}
                            )
                            logger.info(
    ...
    async def list_all_tasks(self, status: str | None = None, limit: int = 20, offset: int = 0) -> list[dict[str, Any]]:
        """获取所有任务列表（不限用户）
        - 合并内存和 MongoDB 数据
        - 按开始时间倒序排列
        """
