...
class BaoStockInitService:
    ...
    async def check_database_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            quotes_latest = None
            if quotes_count > 0:
                latest_doc = await self.db.market_quotes.find_one({"data_source": "baostock"}, sort=[("last_sync", -1)])
                if latest_doc:
    ...
    async def full_initialization(
        self, historical_days: int = 365, force: bool = False, enable_multi_period: bool = False
                ...
                try:
                    ...
                    if financial_data:
                        ...
                        await collection.update_one(
                            {"code": code},
                            {"$set": {"financial_data": financial_data, "financial_data_updated": datetime.now()}},
                        )
                        financial_count += 1
