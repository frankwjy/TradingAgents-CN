...
class BaoStockSyncService:
    ...
    async def _get_total_shares(self, code: str) -> float | None:
        ...
        try:
            ...
            doc = await collection.find_one(
                {"code": code}, {"total_shares": 1, "totalShare": 1}, sort=[("report_period", -1)]
            )
    ...
    async def _update_stock_basic_info(self, basic_info: dict[str, Any]):
        ...
        try:
            ...
            await collection.update_one(
                {"code": basic_info["code"], "source": "baostock"}, {"$set": basic_info}, upsert=True
            )
    ...
    async def _update_stock_quotes(self, quotes: dict[str, Any]):
        ...
        try:
            ...
            # 使用upsert更新或插入
            await collection.update_one({"code": code}, {"$set": quotes}, upsert=True)
    ...
    async def sync_historical_data(
        self, days: int = 30, batch_size: int = 20, period: str = "daily", incremental: bool = True
    ) -> BaoStockSyncStats:
        """
        同步历史数据
    ...
    async def _sync_historical_batch(
        self, code_batch: list[str], days: int, end_date: str, period: str = "daily", incremental: bool = False
    ) -> BaoStockSyncStats:
        """同步历史数据批次"""
        stats = BaoStockSyncStats()
    ...
    async def _update_historical_data(self, code: str, hist_data, period: str = "daily") -> int:
        ...
        try:
            ...
            saved_count = await self.historical_service.save_historical_data(
                symbol=code, data=hist_data, data_source="baostock", market="CN", period=period
            )
