...
class FinancialDataSyncService:
    ...
    async def _sync_source_financial_data(
        self, data_source: str, symbols: list[str], report_types: list[str], batch_size: int, delay_seconds: float
    ) -> FinancialSyncStats:
        """同步单个数据源的财务数据"""
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            logger.info(f"📈 {data_source} 处理批次 {i // batch_size + 1}: {len(batch_symbols)} 只股票")

            # 并发处理批次内的股票
            tasks = []
            for symbol in batch_symbols:
                task = self._sync_symbol_financial_data(
                    symbol=symbol, data_source=data_source, provider=provider, report_types=report_types
                )
                tasks.append(task)
        ...
        try:
            ...
            for report_type in report_types:
                count = await self.financial_service.save_financial_data(
                    symbol=symbol, financial_data=financial_data, data_source=data_source, report_type=report_type
                )
                saved_count += count
    ...
    async def sync_single_stock(self, symbol: str, data_sources: list[str] = None) -> dict[str, bool]:
        """同步单只股票的财务数据"""
        if self.db is None:
            await self.initialize()
        ...
        for data_source in data_sources:
            ...
            try:
                ...
                result = await self._sync_symbol_financial_data(
                    symbol=symbol, data_source=data_source, provider=provider, report_types=["quarterly"]
                )
