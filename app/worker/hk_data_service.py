...
class HKDataService:
    ...
    async def get_stock_info(
        self, stock_code: str, source: str | None = None, force_refresh: bool = False
    ) -> dict[str, Any] | None:
        """
        获取港股基础信息（按需获取+缓存）
    ...
    async def _get_cached_info(self, code: str, source: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            cached = await self.db.stock_basic_info_hk.find_one(
                {"code": code, "source": source, "updated_at": {"$gte": cache_expire_time}}
            )
    ...
    async def _save_to_cache(self, stock_info: dict[str, Any]) -> bool:
        ...
        try:
            await self.db.stock_basic_info_hk.update_one(
                {"code": stock_info["code"], "source": stock_info["source"]}, {"$set": stock_info}, upsert=True
            )
            return True
