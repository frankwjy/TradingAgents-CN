...
class MultiPeriodSyncService:
        ...
        try:
            ...
            for data_source in data_sources:
                for period in periods:
                    period_stats = await self._sync_period_data(data_source, period, symbols, start_date, end_date)
                    ...
                    # 进度日志
                    logger.info(f"📊 {data_source}-{period}同步完成: {period_stats.get('records', 0)}条记录")
    ...
    async def _sync_period_data(
        self, data_source: str, period: str, symbols: list[str], start_date: str = None, end_date: str = None
    ) -> dict[str, Any]:
        """同步特定周期的数据"""
    ...
    async def _sync_batch_period_data(
        self, service, data_source: str, period: str, symbols: list[str], start_date: str = None, end_date: str = None
    ) -> dict[str, Any]:
        """同步批次周期数据"""
        stats = {"records": 0, "success": 0, "errors": 0}
        ...
        for symbol in symbols:
            try:
                ...
                if data_source == "tushare" or data_source == "akshare" or data_source == "baostock":
                    hist_data = await service.provider.get_historical_data(symbol, start_date, end_date, period)
                else:
                    continue
                ...
                if hist_data is not None and not hist_data.empty:
                    ...
                    saved_count = await self.historical_service.save_historical_data(
                        symbol=symbol, data=hist_data, data_source=data_source, market="CN", period=period
                    )
    ...
    async def get_sync_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            for result in results:
                ...
                stats[period][source] = {"count": result["count"], "latest_date": result["latest_date"]}

            return {"period_statistics": stats, "last_updated": datetime.utcnow().isoformat()}
