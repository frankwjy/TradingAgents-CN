...
class NewsDataSyncService:
    ...
    async def sync_stock_news(
        self, symbol: str, data_sources: list[str] = None, hours_back: int = 24, max_news_per_source: int = 50
    ) -> NewsSyncStats:
        """
        同步单只股票的新闻数据
        ...
        try:
            ...
            if "akshare" in data_sources:
                try:
                    akshare_news = await self._sync_akshare_news(symbol, hours_back, max_news_per_source)
                    if akshare_news:
    ...
    async def _sync_akshare_news(self, symbol: str, hours_back: int, max_news: int) -> list[dict[str, Any]]:
        """同步AKShare新闻"""
        try:
    ...
    async def sync_market_news(
        self, data_sources: list[str] = None, hours_back: int = 24, max_news_per_source: int = 100
    ) -> NewsSyncStats:
        """
        同步市场新闻
