...
try:
    ...
    pytest.skip("app dependencies not available", allow_module_level=True)
...
def test_news_ok_with_announcements_and_source(client):
    ...
    with patch(
        "app.services.data_sources.manager.DataSourceManager.get_news_with_fallback", return_value=(items, "tushare")
    ):
