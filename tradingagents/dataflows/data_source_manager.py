...
class DataSourceManager:
    ...
    def _get_cached_data(
        self, symbol: str, start_date: str = None, end_date: str = None, max_age_hours: int = 24
    ) -> pd.DataFrame | None:
        """
        从缓存获取数据
    ...
    def get_stock_dataframe(
        self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily"
    ) -> pd.DataFrame:
        """
...
class USDataSourceManager:
    ...
    def _check_available_sources(self) -> list[USDataSource]:
        ...
        if "alpha_vantage" in enabled_sources_in_db:
            try:
                # 优先从数据库配置读取 API Key，其次从环境变量读取
                api_key = datasource_configs.get("alpha_vantage", {}).get("api_key") or os.getenv(
