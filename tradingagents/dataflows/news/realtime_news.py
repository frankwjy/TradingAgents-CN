                ...
                try:
                    ...
                    if hasattr(entry, "published_parsed") and entry.published_parsed:
                        publish_time = datetime.fromtimestamp(
                            time.mktime(entry.published_parsed), tz=ZoneInfo(get_timezone_name())
                        )
                    else:
    ...
    if is_china_stock:
        ...
        try:
            ...
            if not news_df.empty:
                ...
                logger.info(
                    f"[新闻分析] 成功生成 {ticker} 的新闻报告，总耗时 {total_time_taken:.2f} 秒，新闻来源: 东方财富"
                )
                ...
                logger.warning(
                    f"[新闻分析] 东方财富未获取到 {ticker} 的新闻，耗时 {time_taken:.2f} 秒，尝试使用其他新闻源"
                )
        except Exception as e:
