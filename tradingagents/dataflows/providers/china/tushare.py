...
class TushareProvider(BaseStockDataProvider):
    ...
    async def get_historical_data(
        self, symbol: str, start_date: str | date, end_date: str | date = None, period: str = "daily"
    ) -> pd.DataFrame | None:
        """
        获取历史数据
    ...
    async def get_financial_data(
        self, symbol: str, report_type: str = "quarterly", period: str = None, limit: int = 4
    ) -> dict[str, Any] | None:
        """
        获取财务数据
            ...
            for source in sources_to_try:
                try:
                    ...
                    news_df = await asyncio.to_thread(
                        self.api.news, src=source, start_date=start_date, end_date=end_date
                    )
    ...
    async def get_financial_data_by_period(
        self, symbol: str, start_period: str = None, end_period: str = None, report_type: str = "quarterly"
    ) -> list[dict[str, Any]] | None:
        """
        按时间范围获取财务数据
    ...
    def _standardize_tushare_financial_data(self, financial_data: dict[str, Any], ts_code: str) -> dict[str, Any]:
        ...
        try:
            ...
            symbol = ts_code.split(".")[0] if "." in ts_code else ts_code
            report_period = (
                latest_income.get("end_date") or latest_balance.get("end_date") or latest_cashflow.get("end_date")
            )
            ann_date = (
