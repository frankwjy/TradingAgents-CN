...
class HKNewsProvider:
    ...
    def get_stock_news_sync(self, symbol: str, limit: int = 10) -> Optional[pd.DataFrame]:
        """
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"⚠️ [港股新闻] {symbol} 第{attempt+1}次获取失败(JSON解析错误)，{retry_delay}秒后重试...")
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"⚠️ [港股新闻] {symbol} 第{attempt+1}次获取失败: {e}，{retry_delay}秒后重试...")
                        time.sleep(retry_delay)
                        retry_delay *= 2
                    else:
                        raise
    ...
    @staticmethod
    def _parse_news_time(time_str: str) -> Optional[datetime]:
        """解析新闻时间"""
        if not time_str:
...
def get_hk_stock_news_sync(symbol: str, limit: int = 10) -> Optional[pd.DataFrame]:
    """便捷函数：同步获取港股新闻"""
